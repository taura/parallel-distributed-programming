...
\begin{lstlisting}
$ nvc -Wall @\ao{\tt -mp -target=gpu}@ program.c
\end{lstlisting}% $
\end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item parallelize
    ...
    \item \ao{\tt teams} and \ao{\tt distribute}
      \begin{itemize}
      \item {\tt teams} : creates a number of teams executing the same statement
        ($\approx$ {\tt parallel} pragma)
      \item {\tt distributed} : distribute iterations of a for loop among teams
        ($\approx$ {\tt for} pragma)
      \end{itemize}
    \item \ao{\tt parallel} and \ao{\tt for}
      \begin{itemize}
      \item {\tt parallel} : creates a number of threads
        executing the same statement in a team
      \item {\tt for} : distribute iterations of a for loop among threads
        in a team
      \end{itemize}
    \item think of {\tt teams} $+$ {\tt distributed} another layer
      outside {\tt parallel} $+$ {\tt for}
    \end{itemize}
...
f<<<nb,bs>>>(a, b, c, ...);
\end{lstlisting}
  \item gettint the result back from GPU is already painful
\begin{lstlisting}
cudaMalloc(&r_dev, ...);
f<<<nb,bs>>>(a, b, c, ..., r);
cudaMemcpy(r, r_dev, ...);
...
%%%%%%%%%%%%%%%%% 
\begin{frame}[fragile]
  \frametitle{Data mapping example}
  \begin{itemize}
  \item 
\begin{lstlisting}
#pragma target data @\ao{\tt map(to: a[b:c]) map(from: x)}@
   @$S$@
\end{lstlisting}
\begin{itemize}
\item send the array range {\tt a[b:c]} ({\tt a[b], a[b+1], \ldots, a[c-1]})
  {\it to} GPU before $S$
\item send {\tt x} {\it from} GPU after $S$
\end{itemize}
\item you can combine \ao{\tt to:} and \ao{\tt from:} into \ao{\tt tofrom:}
\item somewhat ``declarative'' way of understanding this
  \begin{itemize}
  \item expressions {\tt a[$i$]} ({\tt b} $\leq i <$ {\tt c}) become
    valid (``mapped'') on GPU during $S$
  \item expressions {\tt x} become valid on CPU after $S$
  \end{itemize}
\item note: you can specify {\tt map} clauses as part of
  {\tt target} (not {\tt target data}) directive, too
\item learn details with tht notebook
\end{itemize}
\end{frame}
